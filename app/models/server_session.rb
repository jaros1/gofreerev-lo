class ServerSession < ActiveRecord::Base
  ...
  # 3: server_session_id - pseudo session id received in online users message from other Gofreerev server
  validates_presence_of :server_session_id
  validates_uniqueness_of :server_session_id, :allow_blank => true
